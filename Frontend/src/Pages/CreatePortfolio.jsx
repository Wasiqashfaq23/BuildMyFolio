...
function FieldRenderer({ field, value, onChange }) {
      ...
      <div className="flex items-center gap-3">
        <div
          ...
          className={`w-10 h-5 rounded-full cursor-pointer transition relative ${
            value ? "bg-[#f5f5f5]" : "bg-[#222]"
          }`}
        >
          <div
            className={`absolute top-0.5 w-4 h-4 rounded-full transition-all ${
              value ? "bg-[#080808] left-5" : "bg-[#555] left-0.5"
            }`}
          />
        </div>
  ...
  if (field.type === "image") {
    ...
    function handleChange(e) {
      ...
      onChange(
        field.multiple
          ? [...images, ...files].slice(0, field.max || 4)
          : files
      );
    }
  ...
  if (field.type === "array") {
    ...
    return (
      <div className="flex flex-col gap-3">
        {field.instruction && (
          <p className="text-xs text-[#444]">{field.instruction}</p>
        )}
...
function CreatePortfolio() {
  ...
  function handleSubmit() {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/portfolio`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        if (data.slug) navigate(`/p/${data.slug}`);
      });
  }
          ...
          >
            <div
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                i <= currentIndex ? "bg-[#f5f5f5]" : "bg-[#333]"
              }`}
            />
            {section.label}
          </button>
                ...
                {sections.map((_, i) => (
                  <div
                    ...
                    className={`h-0.5 flex-1 rounded-full transition-all ${
                      i <= currentIndex ? "bg-[#f5f5f5]" : "bg-[#1a1a1a]"
                    }`}
                  />
                ))}
              </div>
