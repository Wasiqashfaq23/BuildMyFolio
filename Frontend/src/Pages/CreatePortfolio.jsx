...
function CreatePortfolio() {
  ...
  if (loading) return <PageLoading message="Loading template..." />;
  if (error || !template) return <PageError message={error || "Template not found."} onRetry={() => window.location.reload()} />;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
      <AppNavbar backTo="/templates" backLabel="Templates" />
      ...
      {showPreview && (
        <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 overflow-y-auto">
          <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700/60 px-4 sm:px-6 py-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Review Your Portfolio</h2>
            <button
      ...
      {showSubmitConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="submit-title">
          <div className="absolute inset-0 bg-black/40 dark:bg-black/60" onClick={() => setShowSubmitConfirm(false)} />
          <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700/60 p-6 w-full max-w-sm">
            <h2 id="submit-title" className="text-lg font-bold text-slate-900 dark:text-white mb-2">Create portfolio?</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Your portfolio will be published and visible to anyone with the link.</p>
